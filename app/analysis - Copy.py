...
def generate_charts(user_id, year=None):
    ...
    try:
        ...
        if len(expenses_df) > 0:
            ...
            # DEBUG TEMPORAL
            print("=== DEBUG HEATMAP ===")
            print(f"Filas de gastos: {len(expenses_df)}")
            print(f"Columnas disponibles: {expenses_df.columns.tolist()}")
            print(f"Primeras filas:\n{expenses_df.head()}")
            print(f"Meses únicos: {expenses_df['month'].unique()}")
            # FIN DEBUG
    ...
    print(f"Heatmap generado: {charts.get('heatmap') is not None}")
